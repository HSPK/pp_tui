...
Python port of `packages/tui/src/keys.ts`.

Supports both legacy terminal sequences and Kitty keyboard protocol.
See: https://sw.kovidgoyal.net/kitty/keyboard-protocol/
Reference: https://github.com/sst/opentui/blob/7da92b4088aebfe27b9f691c04163a48821e49fd/packages/core/src/lib/parse.keypress.ts

Symbol keys are also supported, however some ctrl+symbol combos overlap with
ASCII codes, e.g. ctrl+[ = ESC. Those can still be used for ctrl+shift combos.
"""
...
# Runtime string ID; the TypeScript compile-time string union becomes a plain str in Python.
KeyId = str
...
class Key:
    escape: ClassVar[KeyId] = "escape"
    esc: ClassVar[KeyId] = "esc"
    enter: ClassVar[KeyId] = "enter"
    return_: ClassVar[KeyId] = "return"
    tab: ClassVar[KeyId] = "tab"
    space: ClassVar[KeyId] = "space"
    backspace: ClassVar[KeyId] = "backspace"
    delete: ClassVar[KeyId] = "delete"
    insert: ClassVar[KeyId] = "insert"
    clear: ClassVar[KeyId] = "clear"
    home: ClassVar[KeyId] = "home"
    end: ClassVar[KeyId] = "end"
    page_up: ClassVar[KeyId] = "pageUp"
    page_down: ClassVar[KeyId] = "pageDown"
    up: ClassVar[KeyId] = "up"
    down: ClassVar[KeyId] = "down"
    left: ClassVar[KeyId] = "left"
    right: ClassVar[KeyId] = "right"
    f1: ClassVar[KeyId] = "f1"
    f2: ClassVar[KeyId] = "f2"
    f3: ClassVar[KeyId] = "f3"
    f4: ClassVar[KeyId] = "f4"
    f5: ClassVar[KeyId] = "f5"
    f6: ClassVar[KeyId] = "f6"
    f7: ClassVar[KeyId] = "f7"
    f8: ClassVar[KeyId] = "f8"
    f9: ClassVar[KeyId] = "f9"
    f10: ClassVar[KeyId] = "f10"
    f11: ClassVar[KeyId] = "f11"
    f12: ClassVar[KeyId] = "f12"

    backtick: ClassVar[KeyId] = "`"
    hyphen: ClassVar[KeyId] = "-"
    equals: ClassVar[KeyId] = "="
    left_bracket: ClassVar[KeyId] = "["
    right_bracket: ClassVar[KeyId] = "]"
    backslash: ClassVar[KeyId] = "\\"
    semicolon: ClassVar[KeyId] = ";"
    quote: ClassVar[KeyId] = "'"
    comma: ClassVar[KeyId] = ","
    period: ClassVar[KeyId] = "."
    slash: ClassVar[KeyId] = "/"
    exclamation: ClassVar[KeyId] = "!"
    at: ClassVar[KeyId] = "@"
    hash: ClassVar[KeyId] = "#"
    dollar: ClassVar[KeyId] = "$"
    percent: ClassVar[KeyId] = "%"
    caret: ClassVar[KeyId] = "^"
    ampersand: ClassVar[KeyId] = "&"
    asterisk: ClassVar[KeyId] = "*"
    left_paren: ClassVar[KeyId] = "("
    right_paren: ClassVar[KeyId] = ")"
    underscore: ClassVar[KeyId] = "_"
    plus: ClassVar[KeyId] = "+"
    pipe: ClassVar[KeyId] = "|"
    tilde: ClassVar[KeyId] = "~"
    left_brace: ClassVar[KeyId] = "{"
    right_brace: ClassVar[KeyId] = "}"
    colon: ClassVar[KeyId] = ":"
    less_than: ClassVar[KeyId] = "<"
    greater_than: ClassVar[KeyId] = ">"
    question: ClassVar[KeyId] = "?"

    @staticmethod
    def ctrl(key: KeyId) -> KeyId:
        return f"ctrl+{key}"

    @staticmethod
    def shift(key: KeyId) -> KeyId:
        return f"shift+{key}"

    @staticmethod
    def alt(key: KeyId) -> KeyId:
        return f"alt+{key}"

    @staticmethod
    def super_(key: KeyId) -> KeyId:
        return f"super+{key}"

    @staticmethod
    def ctrl_shift(key: KeyId) -> KeyId:
        return f"ctrl+shift+{key}"

    @staticmethod
    def shift_ctrl(key: KeyId) -> KeyId:
        return f"shift+ctrl+{key}"

    @staticmethod
    def ctrl_alt(key: KeyId) -> KeyId:
        return f"ctrl+alt+{key}"

    @staticmethod
    def alt_ctrl(key: KeyId) -> KeyId:
        return f"alt+ctrl+{key}"

    @staticmethod
    def shift_alt(key: KeyId) -> KeyId:
        return f"shift+alt+{key}"

    @staticmethod
    def alt_shift(key: KeyId) -> KeyId:
        return f"alt+shift+{key}"

    @staticmethod
    def ctrl_super(key: KeyId) -> KeyId:
        return f"ctrl+super+{key}"

    @staticmethod
    def super_ctrl(key: KeyId) -> KeyId:
        return f"super+ctrl+{key}"

    @staticmethod
    def shift_super(key: KeyId) -> KeyId:
        return f"shift+super+{key}"

    @staticmethod
    def super_shift(key: KeyId) -> KeyId:
        return f"super+shift+{key}"

    @staticmethod
    def alt_super(key: KeyId) -> KeyId:
        return f"alt+super+{key}"

    @staticmethod
    def super_alt(key: KeyId) -> KeyId:
        return f"super+alt+{key}"

    @staticmethod
    def ctrl_shift_alt(key: KeyId) -> KeyId:
        return f"ctrl+shift+alt+{key}"

    @staticmethod
    def ctrl_shift_super(key: KeyId) -> KeyId:
...
_KITTY_CSI_U_PATTERN = re.compile(r"^\x1b\[(\d+)(?::(\d*))?(?::(\d+))?(?:;(\d+))?(?::(\d+))?u$")
_KITTY_ARROW_PATTERN = re.compile(r"^\x1b\[1;(\d+)(?::(\d+))?([ABCD])$")
_KITTY_FUNCTIONAL_PATTERN = re.compile(r"^\x1b\[(\d+)(?:;(\d+))?(?::(\d+))?~$")
_KITTY_HOME_END_PATTERN = re.compile(r"^\x1b\[1;(\d+)(?::(\d+))?([HF])$")
_MODIFY_OTHER_KEYS_PATTERN = re.compile(r"^\x1b\[27;(\d+);(\d+)~$")
...
def is_key_release(data: str) -> bool:
    """Return whether ``data`` looks like a Kitty key release event."""
    if "\x1b[200~" in data:
        return False
...
def is_key_repeat(data: str) -> bool:
    """Return whether ``data`` looks like a Kitty key repeat event."""
    if "\x1b[200~" in data:
        return False
...
def parse_kitty_sequence(data: str) -> ParsedKittySequence | None:
    ...
    csi_u_match = _KITTY_CSI_U_PATTERN.match(data)
    if csi_u_match:
        codepoint = int(csi_u_match.group(1))
        shifted_key_str = csi_u_match.group(2)
        shifted_key = int(shifted_key_str) if shifted_key_str else None
    ...
    arrow_match = _KITTY_ARROW_PATTERN.match(data)
    if arrow_match:
    ...
    func_match = _KITTY_FUNCTIONAL_PATTERN.match(data)
    if func_match:
        ...
        if codepoint is not None:
            _last_event_type = event_type
            return ParsedKittySequence(codepoint=codepoint, modifier=mod_value - 1, event_type=event_type)

    home_end_match = _KITTY_HOME_END_PATTERN.match(data)
    if home_end_match:
...
def _symbol_char(codepoint: int) -> str | None:
...
def matches_kitty_sequence(data: str, expected_codepoint: int, expected_modifier: int) -> bool:
    ...
    if parsed.base_layout_key is not None and parsed.base_layout_key == expected_codepoint:
        ...
        is_latin_letter = 97 <= codepoint <= 122
        symbol = _symbol_char(codepoint)
        is_known_symbol = symbol in SYMBOL_KEYS if symbol is not None else False
...
def parse_modify_other_keys_sequence(data: str) -> ParsedModifyOtherKeysSequence | None:
    match = _MODIFY_OTHER_KEYS_PATTERN.match(data)
    if match is None:
        return None
...
def is_windows_terminal_session() -> bool:
    ...
    return (
        ...
        and not env.get("SSH_CONNECTION")
        and not env.get("SSH_CLIENT")
        and not env.get("SSH_TTY")
    )
...
def matches_printable_modify_other_keys(data: str, expected_keycode: int, expected_modifier: int) -> bool:
    ...
    ) == normalize_shifted_letter_identity_codepoint(
        expected_keycode,
        expected_modifier,
    )
...
def parse_key_id(key_id: str) -> ParsedKeyId | None:
...
def matches_key(data: str, key_id: KeyId) -> bool:
    ...
    if key == "clear":
        return (
            matches_legacy_sequence(data, LEGACY_KEY_SEQUENCES["clear"])
            if modifier == 0
            else matches_legacy_modifier_sequence(data, "clear", modifier)
        )
    ...
    if key == "pageup":
        if modifier == 0:
            return matches_legacy_sequence(data, LEGACY_KEY_SEQUENCES["pageUp"]) or matches_kitty_sequence(
    ...
    if key == "pagedown":
        if modifier == 0:
            return matches_legacy_sequence(data, LEGACY_KEY_SEQUENCES["pageDown"]) or matches_kitty_sequence(
    ...
    if len(key) == 1 and (("a" <= key <= "z") or is_digit_key(key) or key in SYMBOL_KEYS):
        ...
        if modifier == MODIFIERS["ctrl"]:
            if raw_ctrl is not None and data == raw_ctrl:
                return True
...
def format_parsed_key(codepoint: int, modifier: int, base_layout_key: int | None = None) -> str | None:
    ...
    symbol = _symbol_char(identity_codepoint)
    is_known_symbol = symbol in SYMBOL_KEYS if symbol is not None else False
    effective_codepoint = (
        identity_codepoint
    ...
    if effective_codepoint == CODEPOINTS["escape"]:
        ...
        symbol = _symbol_char(effective_codepoint)
        if symbol in SYMBOL_KEYS if symbol is not None else False:
            key_name = symbol
...
def parse_key(data: str) -> str | None:
    ...
    if legacy_sequence_key_id is not None:
...
def decode_kitty_printable(data: str) -> str | None:
    match = _KITTY_CSI_U_PATTERN.match(data)
    if match is None:
        return None

    codepoint = int(match.group(1))
    shifted_key_str = match.group(2)
    shifted_key = int(shifted_key_str) if shifted_key_str else None
    ...
    effective_codepoint = shifted_key if (modifier & MODIFIERS["shift"] and shifted_key is not None) else codepoint
