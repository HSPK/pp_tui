...
class TestFuzzyMatch:
    def test_empty_query_matches_everything_with_score_0(self):
    ...
    def test_consecutive_matches_score_better_than_scattered(self):
    ...
    def test_word_boundary_matches_score_better(self):
    ...
    def test_matches_swapped_alpha_numeric_tokens(self):
        result = fuzzy_match("codex52", "gpt-5.2-codex")
        assert result.matches is True


class TestFuzzyFilter:
    def test_empty_query_returns_all_items_unchanged(self):
        items = ["apple", "banana", "cherry"]
        result = fuzzy_filter(items, "", lambda x: x)
        assert result == items

    def test_filters_out_non_matching_items(self):
        items = ["apple", "banana", "cherry"]
    ...
    def test_sorts_results_by_match_quality(self):
        items = ["a_p_p", "app", "application"]
        result = fuzzy_filter(items, "app", lambda x: x)

        assert result[0] == "app"

    def test_prioritizes_exact_matches_over_longer_prefix_matches(self):
        items = ["clone", "cl"]
        result = fuzzy_filter(items, "cl", lambda x: x)

        assert result == ["cl", "clone"]

    def test_works_with_custom_get_text_function(self):
        items = [
            {"name": "foo", "id": 1},
            {"name": "bar", "id": 2},
            {"name": "foobar", "id": 3},
    ...
    def test_matches_slash_separated_provider_model_queries(self):
